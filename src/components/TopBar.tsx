...
// filenames. strips umlauts too, cant be bothered with a transliteration table
...
export default function TopBar({ canvasRef, onOpenProjects }: Props) {
  ...
  // 1400ms of "Saved ✓" then back. no toast lib for one label
  const doSave = () => {
    saveProject();
    setSaved(true);
  ...
  const onImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    input.value = ''; // clear now, else picking the same file twice does nothing
    if (!file) return;
    ...
    reader.onload = () => {
      ...
      const bud = parsed.budget ?? parsed; // bare budget or full project, both fine
      if (Array.isArray(bud.sources) && Array.isArray(bud.categories)) {
        ...
        if (parsed.settings) updateSettings(parsed.settings); // came from a "+ style" export
        ...
        setBudget(migrateModelToBudget(parsed as SankeyModel)); // ancient format, still turns up
        flashImported();
      } else {
