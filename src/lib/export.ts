// svg/png/json export. no libs, its all built in
...
// clone + stamp xmlns/width/height or nothing outside the browser can open it
...
export function exportSVG(svg: SVGSVGElement, name: string) {
  ...
  setTimeout(() => URL.revokeObjectURL(url), 1000); // revoke too early and firefox drops the download
}
...
export function exportPNG(svg: SVGSVGElement, name: string, scale = 2): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      ctx.scale(scale, scale); // scale the ctx, not the img, keeps text sharp
